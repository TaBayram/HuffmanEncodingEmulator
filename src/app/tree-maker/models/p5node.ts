...
export class p5Node {
    ...
    constructor(public node: TreeNode, public x: number, public y: number, public size: number, public color: Color) { }
    draw(p5: p5) {
        for (const line of this.lines) {
            p5.line(this.x, this.y, line.vector.x, line.vector.y);
            p5.fill(0);
            p5.text(line.text, (this.x + line.vector.x) / 2 + line.textOffset, (this.y + line.vector.y) / 2);
        }
    ...
    calculatePosition(startPosition: Vector, distance: number, angle: number) {
        ...
        this.leftNode?.calculatePosition(this.PolarProjection(distance, 90 + angle), distance, angle);
        this.rightNode?.calculatePosition(this.PolarProjection(distance, 90 - angle), distance, angle);

        if (this.leftNode) {
            this.lines.push({ text: "0", textOffset: -10, vector: { x: this.leftNode.x, y: this.leftNode.y } });
        }
        if (this.rightNode) {
            this.lines.push({ text: "1", textOffset: 10, vector: { x: this.rightNode.x, y: this.rightNode.y } });
        }
    }
